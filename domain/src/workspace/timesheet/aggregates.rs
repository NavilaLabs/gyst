...
impl Aggregate {
    ...
    #[must_use]
    pub const fn activity_id(&self) -> Option<&activity::Id> {
...
impl eventually::aggregate::Aggregate for Aggregate {
    type Id = timesheet::Id;
    type Event = timesheet::Event;
    ...
    fn apply(state: Option<Self>, event: Self::Event) -> Result<Self, Self::Error> {
        match (state, event) {
            ...
            (Some(_), timesheet::Event::Started { .. }) => Err(timesheet::Error::AlreadyExists),
            (None, _) => Err(timesheet::Error::NotFound),
            (
                Some(mut t),
                timesheet::Event::Stopped {
                    end_time, duration, ..
                },
            ) => {
            ...
            (Some(mut t), timesheet::Event::Updated { description }) => {
                t.description = description;
                Ok(t)
            }
            (Some(mut t), timesheet::Event::Reassigned { activity_id }) => {
                t.activity_id = Some(activity_id);
                Ok(t)
            }
            (
                Some(mut t),
                timesheet::Event::TimeUpdated {
                    start_time,
                    end_time,
                    duration,
                },
            ...
            (Some(t), timesheet::Event::Cancelled {}) => {
                if t.cancelled {
                    return Err(timesheet::Error::AlreadyCancelled);
                }
                Ok(Self {
                    cancelled: true,
                    ..t
...
mod tests {
    ...
    fn ts_id() -> timesheet::Id {
        TS_ID.parse().unwrap()
    }
    fn user_id() -> user::Id {
        USER_ID.parse().unwrap()
    }
    fn act_id() -> activity::Id {
        ACT_ID.parse().unwrap()
    }

    fn started_event() -> timesheet::Event {
        timesheet::Event::Started {
            id: ts_id(),
            user_id: user_id(),
    ...
    fn started() -> timesheet::Aggregate {
        Aggregate::apply(None, started_event()).unwrap()
    }
    ...
    fn apply_started_without_activity_id_is_valid() {
        let t = timesheet::Aggregate::apply(
            None,
            timesheet::Event::Started {
                id: ts_id(),
                user_id: user_id(),
    ...
    fn apply_started_to_existing_returns_already_exists() {
        ...
        let result = Aggregate::apply(Some(existing), started_event());
        assert!(matches!(result, Err(timesheet::Error::AlreadyExists)));
    }

    #[test]
    fn apply_non_started_event_to_no_state_returns_not_found() {
        let result = timesheet::Aggregate::apply(
            None,
            timesheet::Event::Stopped {
        ...
        assert!(matches!(result, Err(timesheet::Error::NotFound)));
    }

    #[test]
    fn apply_stopped_sets_end_time_and_duration() {
        let t = Aggregate::apply(
            Some(started()),
            timesheet::Event::Stopped {
                end_time: "2024-01-01T10:00:00Z".to_string(),
                duration: 3600,
    ...
    fn apply_updated_can_clear_description() {
        ...
        .unwrap();
        let cleared = Aggregate::apply(
            Some(with_desc),
            timesheet::Event::Updated { description: None },
        )
        .unwrap();
    ...
    fn apply_reassigned_sets_activity_id() {
        let new_act: activity::Id = "019d0ce8-facb-7c90-b9d7-287ae4f17c94".parse().unwrap();
        let t = Aggregate::apply(
            Some(started()),
            timesheet::Event::Reassigned {

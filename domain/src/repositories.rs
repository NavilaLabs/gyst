...
pub trait ProjectionToRoot<R, T>
where
    T: Aggregate,
{
    ...
    fn projection_to_root(&self, row: R) -> Result<Root<T>, Self::Error>;
}

#[async_trait]
pub trait ReadRepository<T, R>: Send + Sync + Getter<T> + ProjectionToRoot<R, T>
where
    T: Aggregate,
{
    type Error: Debug + Send + Sync + From<GetError> + From<<Self as ProjectionToRoot<R, T>>::Error>;

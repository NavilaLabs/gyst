...
mod tests {
    ...
    fn ensure_sqlite_extension_appends_sqlite_suffix() {
        let result = TestUri
            .ensure_sqlite_extension("sqlite", uri("sqlite:///tmp/zeitrak_admin"))
            .unwrap();
    ...
    fn ensure_sqlite_extension_does_not_double_append() {
        let already = uri("sqlite:///tmp/zeitrak_admin.sqlite");
    ...
    fn ensure_sqlite_extension_does_not_modify_postgres_uri() {
        let pg = uri("postgres://localhost/zeitrak_admin");
